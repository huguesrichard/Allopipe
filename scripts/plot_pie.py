...
for file in files:
    ...
    if df is not None and 'CHROM' in df.columns:
        ...
        plt.title(f'Distribution of AMS across chromosoms in {file}') #####################ID PAIRE
        ...
        # Save the pie chart with the table as a JPEG image
        output_path = f'{file.replace(".tsv", "_pie_chart.jpeg")}'
