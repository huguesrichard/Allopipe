...
# read the hla file provided by Adele Dhuyser at the moment, file format may change in the future
def read_hla_hardfile(hla_file, sheet=""):
    """
    ...
    if hla_file.endswith("xlsx"):
        # check if there is a specificied sheet
        if sheet != "":
            hla_df = pd.read_excel(hla_file, sheet_name=sheet)
        else:
            hla_df = pd.read_excel(hla_file)
    elif hla_file.endswith("tsv") or hla_file.endswith("xls"):
        hla_df = pd.read_csv(hla_file, sep="\t")
    else:
        hla_df = pd.read_csv(hla_file)
    # drop rows with nan on the whole dataframe
    hla_df = hla_df.dropna()
    # pivot the dataframe
    pivoted = pd.pivot_table(hla_df, columns=[hla_df.iloc[:, 0]], aggfunc="first")
    pivoted = pivoted.reset_index()
    # sort the dataframe
    if "Unnamed" in pivoted["index"][0]:
        pivoted["index"] = pivoted["index"].str.replace("Unnamed: ", "").astype(int)
        pivoted = pivoted.sort_values(by="index")
        pivoted = pivoted.drop("index", axis=1)
    # create class column containing either I or II for HLA complex
    pivoted["class"] = np.where(
        pivoted[pivoted.columns[0]].str.contains("HLA-D"), "II", "I"
    )
    # create a drop condition of the unwanted rows
    pivoted["drop"] = np.where(
        pivoted[pivoted.columns[0]].str.contains("HLA"), True, False
    )
    pivoted = pivoted[pivoted["drop"] is True]
    pivoted = pivoted.drop("drop", axis=1)
    pivoted = pivoted.reset_index(drop=True)
    print(pivoted)
    # replace the * character as it is not accepted by netMHCpan
    pivoted = pivoted.apply(lambda x: x.str.replace("*", "", regex=True))
    # split both HLA classes
    class_1 = pivoted[pivoted["class"] == "I"].copy()
    class_2 = pivoted[pivoted["class"] == "II"].copy()
    # join HLAs
    joined_class_1 = class_1.astype(str).apply(",".join).reset_index()
    joined_class_2 = class_2.astype(str).apply(",".join).reset_index()
    print(joined_class_2)
    # pivot dataframes
    class_1 = pd.pivot_table(joined_class_1, columns=["Unnamed: 0"], aggfunc="first")
    class_2 = pd.pivot_table(joined_class_2, columns=["Unnamed: 0"], aggfunc="first")
    class_1 = class_1.drop("class", axis=1)
    class_2 = class_2.drop("class", axis=1)
    print(class_1, class_2)
    print(pivoted)
    return (class_1, class_2, pivoted)


def read_hla_file(hla_file, sheet=""):
    """
    ...
    if hla_file.endswith("xlsx"):
        ...
        hla_df = pd.read_csv(hla_file)
    hla_df = hla_df.dropna()
    # drop the patient sex
    # print(hla_df)
    hla_df = hla_df.drop(0)
    hla_df = hla_df.reset_index(drop=True)
    hla_df["class"] = np.where(
        hla_df[hla_df.columns[0]].str.contains("HLA-D"), "II", "I"
    )
    # replace the * character as it is not accepted by netMHCpan
    hla_df = hla_df.apply(lambda x: x.str.replace("*", "", regex=True))
    # split both HLA classes
    class_1 = hla_df[hla_df["class"] == "I"].copy()
    class_2 = hla_df[hla_df["class"] == "II"].copy()
    # join HLAs
    joined_class_1 = class_1.astype(str).apply(",".join).reset_index()
    joined_class_2 = class_2.astype(str).apply(",".join).reset_index()
    class_1 = pd.pivot_table(joined_class_1, columns=["index"], aggfunc="first")
    class_2 = pd.pivot_table(joined_class_2, columns=["index"], aggfunc="first")
    class_1 = class_1.drop(["class", "Nom échantillon"], axis=1)
    class_2 = class_2.drop(["class", "Nom échantillon"], axis=1)
    return (class_1, class_2, hla_df)
