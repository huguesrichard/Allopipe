...
def pie(run_tables, run_plots, pair, run_name):
    ...
    # Find the specific file corresponding to the given pair
    file = next((f for f in os.listdir(directory) if f.endswith('.tsv') and 'mismatches' in f and pair in f), None)

    if not file:
        print(f"No mismatch file found for pair: {pair}")
        return
